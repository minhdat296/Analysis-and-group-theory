\section{Normed spaces}
    ...
    \subsection{Hilbert spaces; some harmonic analysis}
        ...
        \begin{definition}[Hilbert spaces] \label{def: hilbert_spaces}
            ...
            A \textbf{Hilbert space} is a complete Hausdorff topological vector space space $H$ equipped with a $\sigma$-braided and positive-definite $K$-bilinear form $\<-, -\>_H: H \x H \to K$ (called an \textbf{inner product}), with the braidedness condition meaning that:
        ...
        \end{definition}
        \begin{proposition}[Induced norms] \label{prop: induced_norms}
        ...
        \begin{theorem}[Riesz's Representability Theorem] \label{theorme: riesz_representation_theorem}
            Let $H$ be a Hilbert space. Then, any linear functional $\varphi \in H^*$ will be representable, in the sense that there exists a unique $y \in H$ such that:
                $$\varphi = \<-, y\>_H$$
            In other words, the inner product of any Hilbert space is non-degenerate, and for all $y \in H$, the linear functional $\<-, y\>_H$ is continuous with respect to the topology generated by $\norm{-}_H$, and one has a linear isometry:
                $$H \xrightarrow[]{\cong} H^*_{\cont}$$
            which is given by $y \mapsto \<-, y\>_H$.
        \end{theorem}
            \begin{proof}
                Let $\varphi \in H^*$ and $y \in H$ be arbitrary. Our goal shall be to show that $y$ can be such that:
                    $$(\varphi - \<-, y\>_H)(x) = 0$$
        ...
        \end{example}
        \begin{example}[$L^2$-space over a compact Lie group]
