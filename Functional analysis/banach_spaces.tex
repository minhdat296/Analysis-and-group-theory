\section{Normed spaces}
    ...
    \subsection{Generalities on topological vector spaces}
        ...
        \end{lemma}
            ...
            \end{proof}
        \begin{theorem}[The Baire Category Theorem\footnote{In general topology, this is usually referred to as the Baire Category Theorem I. We will not be needing the Baire Category II.}] \label{theorem: baire_category}
        ...
        \end{theorem}
            \begin{proof}
                Suppose for the sake of deriving a contradiction that $X$ is a metric space that is complete but not Baire, i.e. there exists a countable collection $\{U_n\}_{n \in \N}$ of dense open subsets of $X$ whose intersection $U := \bigcap_{n \in \N} U_n$ is not dense. A subset of a topological space is dense if and only if it has a non-empty intersection with every non-empty open subset of that topological space. Thus, if $U$ is not dense, then there must exist some $\e > 0$ and $x \in X$ such that:
                    $$\varnothing = U \cap \B_{\e}(x) = \left(\bigcap_{n \in \N} U_n\right) \cap \B_{\e}(x) = \bigcap_{n \in \N} U_n \cap \B_{\e}(x)$$
                Despite this, each $U_n$ is dense by assumption, so $U_n \cap \B_{\e}(x) \not = \varnothing$ for all $n \in \N$ and all $\e > 0$ and all $x \in X$.
            \end{proof}
        \begin{remark}
        ...
        \begin{theorem}[Closed Graph Theorem] \label{theorem: closed_graph}
